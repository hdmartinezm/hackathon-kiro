...
const audioElements = html.match(/<audio[^>]*>/g) || [];
test('Tiene al menos un elemento <audio>', audioElements.length > 0,
  `Encontrados: ${audioElements.length}`);

audioElements.forEach((audio, i) => {
  const srcMatch = audio.match(/src="([^"]+)"/);
  if (srcMatch) {
    const audioPath = path.join(__dirname, srcMatch[1]);
    test(`Audio ${i+1} (${srcMatch[1]}) existe en disco`, fs.existsSync(audioPath),
      `Archivo no encontrado: ${audioPath}`);
  }
  test(`Audio ${i+1} tiene data-start`, /data-start="\d+"/.test(audio));
  test(`Audio ${i+1} tiene data-duration`, /data-duration="\d+"/.test(audio));
  test(`Audio ${i+1} tiene data-volume`, /data-volume="[\d.]+"/.test(audio));
});
